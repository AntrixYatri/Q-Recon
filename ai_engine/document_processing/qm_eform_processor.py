...
class QMEFormProcessor(BaseDocumentExtractor):
    """
    QM E-Form Extractor utilizing EasyOCR, layout-aware column segmenting,
    section detection, table cell clustering, and checkbox status parsing.
    Supports PDF processing with PyMuPDF.
    """
    def extract(self, document_path_or_dict) -> dict:
        # 1. Structured input bypass (Task 6)
        ...
        try:
            ...
            # Column segmenting above split_y (left vs right columns)
            ...
            # 4. Run Key-Value, Table and Checkbox extraction
            ...
            for r_key, r_val in best_row.items():
                if r_val:
                    if r_key in ["parameter", "measured_value", "required_value", "unit", "quality_status"]:
                        # If it is quality status, it might contain checkboxes, so parse it
                        if r_key == "quality_status":
                            parsed_stat = parse_checkbox_status(r_val)
                            if parsed_stat:
                                extracted_fields[r_key] = parsed_stat
                        else:
                            extracted_fields[r_key] = r_val

            # 5. Extract Checkbox compliance status from lines (Variant A / C checks)
            # Find status or checklist markings in lines
            checkbox_status = None
            for line in lines:
                status_cand = parse_checkbox_status(line["text"])
                if status_cand:
                    # Verify if it is inside the quality/status section
                    sect = get_section_for_yc(line["y"], sections)
                    if sect in ["quality", "remarks", "unknown"]:
                        checkbox_status = status_cand
                        break

            # Fill quality_status if not already populated or if it is empty
            if checkbox_status and not extracted_fields.get("quality_status"):
                extracted_fields["quality_status"] = checkbox_status

            # Ambiguity handling for measured_value
            ambiguities = {}
            measured_candidates = []
            for crow in candidate_rows:
